      ...
      <div className="page-max">
        <div className="phil-layout">
          <div>
            ...
            <p style={{ fontFamily: 'Arial', fontSize: 15, color: C.muted, lineHeight: 1.75 }}>These convictions underpin everything: strategy, positioning, hold periods, and how we think about risk.</p>
          </div>
          <div className="beliefs-grid">
            {beliefs.map((b, i) => (
              <div key={i} style={{ borderTop: `2px solid ${i < 2 ? C.navy : C.gold}`, paddingTop: 18 }}>
                ...
                <p style={{ fontFamily: 'Arial', fontSize: 13, color: C.muted, lineHeight: 1.7 }}>{b.body}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="four-col">
          {strategies.map((s, i) => (
            <div key={i} style={{ background: C.bg, padding: 'clamp(20px,3vw,28px) clamp(16px,2.5vw,24px)' }}>
              <div style={{ fontFamily: 'Arial', fontSize: 10, fontWeight: 700, letterSpacing: '0.10em', textTransform: 'uppercase', color: C.gold, marginBottom: 12 }}>{s.label}</div>
              <h3 style={{ fontFamily: "Georgia,serif", fontSize: 18, fontWeight: 400, color: C.navy, lineHeight: 1.3, marginBottom: 12 }}>{s.title}</h3>
              <p style={{ fontFamily: 'Arial', fontSize: 13, color: C.muted, lineHeight: 1.65 }}>{s.body}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section style={{ background: C.navy, padding: 'clamp(56px,8vw,88px) 0' }}>
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '0 clamp(20px,5vw,40px)', textAlign: 'center' }}>
        ...
        <p style={{ fontFamily: 'Arial', fontSize: 15, color: 'rgba(250,249,246,0.62)', lineHeight: 1.75, marginBottom: 32, textWrap: 'pretty' }}>
